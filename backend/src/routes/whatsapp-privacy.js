...
export function whatsAppPrivacyOrigin() {
  const url = new URL(process.env.WHATSAPP_SIGNUP_URL || 'http://localhost:3001');
...
router.get('/data-deletion', (_req, res) => res.type('html').send(page('WhatsApp data requests',
  'You can request removal of the data Florrie received through WhatsApp using Meta’s app data-deletion option, or by emailing us from the address on your Florrie account. Include the business number connected to Florrie. We will verify the request and review the affected records. Disconnecting WhatsApp stops the connection; it does not by itself erase your salon’s existing records.')));

for (const [path, action] of [['/deauthorize', 'deauthorize'], ['/data-deletion', 'delete']]) {
  router.post(path, async (req, res) => {
    try {
      // Only the WhatsApp app's secret can authorize a WhatsApp data action.
      // Never fall back to a different Meta app's credential or a user JWT.
      const verified = verifyMetaSignedRequest(req.body?.signed_request, getAppSecret(), Date.now(), 'whatsapp:user');
      const origin = whatsAppPrivacyOrigin();
      const { data, error } = await supabase.rpc('request_whatsapp_data_action', {
        p_account_hash: verified.accountHash, p_kind: 'user', p_action: action, p_issued_at: verified.issuedAt,
      ...
      res.json({ url: `${origin}/api/whatsapp/privacy/data-deletion/status/${data.confirmation_code}`,
        confirmation_code: data.confirmation_code });
    } catch (error) {
      if (error instanceof InvalidMetaRequest) return res.status(403).json({ error: 'Invalid signed request' });
      logger.error({ action }, 'WhatsApp privacy request could not be saved');
...
router.get('/data-deletion/status/:code', async (req, res) => {
  ...
  try {
    ...
    res.type('html').send(page(complete ? 'WhatsApp data request completed' : 'WhatsApp data request received',
      complete ? 'We have completed the WhatsApp data cleanup recorded for this request. Contact us with your reference if you need more details.'
        : 'We have saved your request. We stop an active connection when the verified Meta identity matches it. Unmatched identities need review. Our team must review the WhatsApp data held in your salon’s records and complete the cleanup. Deletion is not complete yet.', req.params.code));
  } catch {
