...
async function run() {
  console.log('Comeback engine starting...');
  ...
  if (bErr) {
    ...
    console.error('Failed to fetch beauticians:', bErr.message);
    process.exit(1);
  }
    ...
    for (const client of lapsedClients) {
      ...
      try {
        ...
        if (result) {
          ...
          totalNudged++;
          console.log(`✓ Nudged ${firstName} (${client.phone})`);
        } else {
          totalFailed++;
        ...
        console.error(`✗ Error nudging ${client.phone}:`, err.message);
      }
    }
  }

  console.log(`Comeback engine done. ${totalNudged} sent, ${totalSkipped} skipped (already nudged), ${totalFailed} failed.`);
}
