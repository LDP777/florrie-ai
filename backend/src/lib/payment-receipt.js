/** Insert once per provider payment. The unique index is the concurrency guard;
...
export async function insertPaymentReceipt(db, row) {
  ...
  const match = !prior.error && prior.data?.length === 1 && [
    'beautician_id', 'appointment_id', 'client_id', 'amount_cents', 'type',
  ].every(key => (prior.data[0][key] ?? null) === (row[key] ?? null));
