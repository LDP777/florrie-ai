...
export async function claimBillingEvent(event) {
  const claim = { token: randomUUID(), claimed_at: new Date().toISOString() };
  const data = { ...event, billing_claim: claim };
  ...
  const previous = existing?.data?.billing_claim;
  const age = Date.now() - Date.parse(previous?.claimed_at);
  if (!previous?.token || !Number.isFinite(age) || age < LEASE_MS) {
    throw new Error('Billing event is already processing');
  }
  const { data: reclaimed, error: reclaimError } = await supabase.from('stripe_events')
    .update({ data }).eq('id', event.id).eq('data->billing_claim->>token', previous.token)
    .is('processed_at', null).select('id');
  if (reclaimError) throw reclaimError;
  if (!reclaimed?.length) throw new Error('Billing event claim changed');
  return claim;
}

export async function completeBillingEvent(event, claim) {
  const { data, error } = await supabase.from('stripe_events')
    .update({ processed_at: new Date().toISOString() })
    .eq('id', event.id).eq('data->billing_claim->>token', claim.token).select('id');
  if (error) throw error;
  if (!data?.length) throw new Error('Billing event claim was lost');
}

export async function releaseBillingEvent(event, claim) {
  const { error } = await supabase.from('stripe_events').update({
    data: { ...event, billing_claim: { ...claim, claimed_at: new Date(0).toISOString() } },
  })
    .eq('id', event.id).eq('data->billing_claim->>token', claim.token).is('processed_at', null);
  if (error) throw error;
}
