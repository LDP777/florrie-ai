...
const CATEGORIES = [
  {
    ...
    items: [
      ...
      { path: '/smart-schedule', label: "Florrie's Schedule", desc: 'AI-optimised time slots', icon: '🧠' },
  ...
  },
  {
    ...
    items: [
      { path: '/voice', label: 'Ask Florrie', desc: 'Chat with your AI assistant', icon: '✨' },
      { path: '/ai-insights', label: "Florrie's Insights", desc: 'Predictions and trends', icon: '🧠' },
      { path: '/segments', label: 'Client Segments', desc: 'Smart grouping', icon: '🎯' },
      { path: '/churn', label: 'Client Retention', desc: 'At-risk clients', icon: '🛡️' },
      { path: '/demand', label: 'Demand Forecast', desc: 'Capacity planning', icon: '📊' },
      { path: '/escalations', label: 'Florrie Needs You', desc: 'Messages Florrie flagged', icon: '🚨' },
      { path: '/approval-queue', label: "Florrie's Queue", desc: 'Actions awaiting sign-off', icon: '✅' },
              ...
              <div style={S.itemGrid}>
                {visibleItems.map(item => {
                  ...
                  return (
                    <button
                      key={item.path}
                      onClick={() => locked ? handleNav(item.path) : handleNav(item.path)}
                      style={{
                        ...S.item,
                        ...(isActive ? S.itemActive : {}),
                        ...(locked ? { opacity: 0.65, cursor: 'pointer' } : {}),
                      }}
                    >
