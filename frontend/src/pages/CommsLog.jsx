...
const STATUS_STYLES = {
  delivered: { bg: 'var(--success-bg, #E8F5E9)', color: '#2E7D32', label: 'Delivered' },
  ...
  failed: { bg: 'var(--danger-bg, #FDF0EF)', color: '#C62828', label: 'Failed' },
  bounced: { bg: 'var(--danger-bg, #FDF0EF)', color: '#C62828', label: 'Bounced' },
};
...
export default function CommsLog() {
  ...
  return (
    <div style={s.page}>
        ...
        <div style={s.statsPanel}>
          <div style={s.statsGrid}>
            <div style={s.statCard}>
              <span style={{ fontSize: 20, fontWeight: 700, color: 'var(--text, var(--text-primary, #2D2A26))' }}>{stats.total}</span>
              <span style={{ fontSize: 11, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))' }}>Total</span>
            </div>
            <div style={s.statCard}>
              <span style={{ fontSize: 20, fontWeight: 700, color: 'var(--accent, #C76B8A)' }}>{stats.outbound}</span>
              <span style={{ fontSize: 11, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))' }}>Sent</span>
            </div>
            <div style={s.statCard}>
              <span style={{ fontSize: 20, fontWeight: 700, color: '#7B1FA2' }}>{stats.inbound}</span>
              <span style={{ fontSize: 11, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))' }}>Replies</span>
            </div>
            <div style={s.statCard}>
              <span style={{ fontSize: 20, fontWeight: 700, color: deliveryRate >= 90 ? 'var(--success, #5BA97B)' : '#E8A838' }}>{deliveryRate}%</span>
              <span style={{ fontSize: 11, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))' }}>Delivery</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 12, marginTop: 10 }}>
            {CHANNELS.filter(c => c !== 'all').map(c => (
              <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))' }}>
      ...
      <div style={s.msgList}>
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))', fontSize: 13 }}>No messages found</div>
        )}
        {filtered.map(msg => {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--text, var(--text-primary, #2D2A26))' }}>{msg.client}</span>
                      <span style={{ fontSize: 10, color: isInbound ? '#7B1FA2' : 'var(--accent, #C76B8A)' }}>{isInbound ? '← in' : '→ out'}</span>
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {msg.subject}
                    </div>
                  </div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontSize: 11, color: 'var(--text-muted, var(--text-muted, var(--text-muted, #B5AFA8)))' }}>{timeAgo(msg.time)}</div>
