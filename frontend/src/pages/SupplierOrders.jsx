...
const STATUS_STYLES = {
  delivered: { bg: 'var(--success-bg, #E8F5E9)', color: 'var(--success, #5BA97B)', label: 'Delivered' },
  in_transit: { bg: '#FFF3E0', color: '#E65100', label: 'In Transit' },
  ordered: { bg: '#E3F2FD', color: '#1565C0', label: 'Ordered' },
  cancelled: { bg: 'var(--danger-bg, #FDF0EF)', color: 'var(--danger, #D4605C)', label: 'Cancelled' },
};
    ...
    <div style={s.page}>
      ...
      <div style={s.summaryRow}>
        <div style={s.summaryCard}>
          <span style={{ fontSize: 18, fontWeight: 700, color: lowStock.length > 0 ? 'var(--danger, #D4605C)' : 'var(--success, #5BA97B)' }}>{lowStock.length}</span>
        ...
        <div style={s.summaryCard}>
          <span style={{ fontSize: 18, fontWeight: 700, color: 'var(--accent, #C76B8A)' }}>£{totalStockValue.toFixed(0)}</span>
      ...
      {tab === 'stock' && (
        <div style={s.list}>
          {products.map(p => {
            ...
            return (
              <div key={p.id} style={{ ...s.productCard, borderLeft: isLow ? '3px solid var(--danger, #D4605C)' : '3px solid transparent' }}>
                <div style={s.productTop}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: 700, fontSize: 16, color: isLow ? 'var(--danger, #D4605C)' : 'var(--success, #5BA97B)' }}>{p.stock}</div>
            ...
            return (
              <button key={order.id} onClick={() => setExpandedOrder(expanded ? null : order.id)} style={s.orderCard}>
                <div style={s.orderTop}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ padding: '3px 10px', borderRadius: 12, fontSize: 11, fontWeight: 600, background: status.bg, color: status.color, whiteSpace: 'nowrap' }}>{status.label}</span>
            ...
            return (
              <button key={sup.id} onClick={() => setShowSupplier(expanded ? null : sup.id)} style={s.supplierCard}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent, #C76B8A)' }}>{supplierProducts.length} products</span>
                </div>
                {expanded && (
                  <div style={{ marginTop: 12, borderTop: '1px solid var(--card-border, #F0ECE8)', paddingTop: 10 }}>
                    ...
                    {supplierProducts.map(p => (
                      <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: 13 }}>
                        <span>{p.name}</span>
                        <span style={{ color: p.stock <= p.reorderAt ? 'var(--danger, #D4605C)' : 'var(--success, #5BA97B)', fontWeight: 600 }}>{p.stock} {p.unit}</span>
                      </div>
                    ))}
                  </div>
                )}
...
const s = {
  ...
  alertBanner: { display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 10, background: '#FFF3E0', marginBottom: 12 },  // warning/alert colour
  ...
  reorderBtn: { background: 'none', border: 'none', color: 'var(--accent, #C76B8A)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  ...
  addBtn: { padding: '12px', borderRadius: 10, border: '2px dashed var(--card-border, #F0ECE8)', background: 'none', fontSize: 14, fontWeight: 600, color: 'var(--accent, #C76B8A)', cursor: 'pointer', fontFamily: 'inherit' },
