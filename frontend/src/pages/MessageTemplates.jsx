...
const CATEGORIES = [
  ...
  { key: 'marketing', label: 'Marketing', icon: '💌', color: '#C76B8A' },
    ...
    <div style={s.page}>
        ...
        <div style={s.createCard}>
            ...
            {CATEGORIES.map(c => (
              <button
                ...
                style={{
                  ...
                  border: newCategory === c.key ? `1px solid ${c.color}` : '1px solid var(--border, #E8E4E0)',
                  color: newCategory === c.key ? c.color : 'var(--text, #2D2A26)',
                }}
              >
                {c.icon} {c.label}
              </button>
          ...
          <div style={s.triggerRow}>
            {TRIGGERS.map(t => (
              <button
                ...
                style={{
                  ...s.triggerChip,
                  background: newTrigger === t.key ? '#C76B8A' : 'var(--card-bg, #fff)',
                  color: newTrigger === t.key ? '#fff' : 'var(--text, #2D2A26)',
                  border: newTrigger === t.key ? '1px solid #C76B8A' : '1px solid var(--border, #E8E4E0)',
                }}
              >
                {t.label}
              </button>
          ...
          <div style={s.toggleRow}>
            ...
            <button
              ...
              style={{ ...s.toggle, background: newAutoSend ? '#C76B8A' : '#E8E4E0' }}
            >
      ...
      <div style={s.filterRow}>
        <button
          ...
          style={{ ...s.filterChip, background: filter === 'all' ? '#C76B8A' : 'var(--card-bg, #fff)', color: filter === 'all' ? '#fff' : 'var(--text, #2D2A26)', border: filter === 'all' ? '1px solid #C76B8A' : '1px solid var(--border, #E8E4E0)' }}
        >
          All
        </button>
        {CATEGORIES.map(c => (
          <button
            ...
            style={{ ...s.filterChip, background: filter === c.key ? c.color + '18' : 'var(--card-bg, #fff)', color: filter === c.key ? c.color : 'var(--text, #2D2A26)', border: filter === c.key ? `1px solid ${c.color}` : '1px solid var(--border, #E8E4E0)' }}
          >
            {c.icon}
          </button>
        ))}
...
const s = {
  ...
  createCard: { background: 'var(--card-bg, #fff)', borderRadius: 14, padding: 16, marginBottom: 16, border: '1px solid var(--border, #F0ECE8)' },
  ...
  textarea: { width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid var(--border, #F0ECE8)', fontSize: 13, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', resize: 'vertical', marginBottom: 8, background: 'var(--bg, #FAF8F5)', color: 'var(--text, #2D2A26)', lineHeight: 1.5 },
  varRow: { display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 12 },
  varLabel: { fontSize: 11, color: 'var(--text-muted, #AAA5A0)', fontWeight: 500 },
  varChip: { padding: '3px 8px', borderRadius: 6, border: '1px solid var(--border, #E8E4E0)', background: 'var(--bg, #FAF8F5)', fontSize: 11, fontFamily: 'monospace', cursor: 'pointer', color: '#C76B8A' },
  ...
  toggleLabel: { fontSize: 14, fontWeight: 600, color: 'var(--text, #2D2A26)' },
  ...
  previewBox: { background: 'var(--bg, #FAF8F5)', borderRadius: 10, padding: 12, marginBottom: 10 },
  previewLabel: { display: 'block', fontSize: 10, fontWeight: 600, color: 'var(--text-muted, #AAA5A0)', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 },
  ...
  tmplCard: { background: 'var(--card-bg, #fff)', borderRadius: 12, border: '1px solid var(--border, #F0ECE8)', overflow: 'hidden' },
  ...
  tmplName: { display: 'block', fontSize: 14, fontWeight: 600, color: 'var(--text, #2D2A26)' },
  tmplMeta: { display: 'block', fontSize: 11, color: 'var(--text-muted, #AAA5A0)' },
  chevron: { fontSize: 14, color: 'var(--text-muted, #D5D0CB)', flexShrink: 0 },
  tmplBody: { padding: '0 14px 14px' },
  tmplText: { fontSize: 13, lineHeight: 1.5, color: 'var(--text, #5A5550)', margin: '0 0 10px', background: 'var(--bg, #FAF8F5)', padding: '10px 12px', borderRadius: 8 },
  tmplActions: { display: 'flex', gap: 8 },
  actionBtn: { padding: '6px 14px', borderRadius: 8, border: '1px solid var(--border, #E8E4E0)', background: 'transparent', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', color: 'var(--text, #2D2A26)' },
};
