        ...
        <div style={S.grid}>
          {filterPhotos(photos).map((p, i) => (
            <div key={p.id} style={S.gridItem}>
              ...
              {p.type !== 'single' && (
                <span style={{ ...S.typeBadge, background: p.type === 'before' ? 'var(--border, var(--border, var(--border, #E8DDD4)))' : 'var(--accent-light, #F6E7EC)', color: p.type === 'before' ? 'var(--text-secondary, #574A42)' : 'var(--accent, #92405e)' }}>
                  {p.type}
                </span>
              )}
            </div>
...
const S = {
  ...
  pairLabel: { fontSize: 11, fontWeight: 600, color: 'var(--text-muted, var(--text-muted, #6B5D54))', textTransform: 'uppercase', letterSpacing: '0.05em' },
